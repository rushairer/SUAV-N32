...
static pt2Filter_t piddFilter[XYZ_AXIS_COUNT];
...
void pid(float deltaT)
{
    ...
    const float _imaxYaw = 228.0f; // 360°
    const float _imaxPR  = 350.0f;

    pidconf.PAngleToRate[ROLL]  = 1.0f;
    pidconf.PAngleToRate[PITCH] = 1.0f;
    pidconf.PAngleToRate[YAW]   = 0.3f;
    pidconf.PRate[ROLL]         = 0.12f;
    pidconf.PRate[PITCH]        = 0.12f;
    pidconf.PRate[YAW]          = 0.3f;
    pidconf.IRate[ROLL]         = 0.25f;
    pidconf.IRate[PITCH]        = 0.25f;
    pidconf.IRate[YAW]          = 0.6f;
    pidconf.DRate[ROLL]         = 0.0040f;
    pidconf.DRate[PITCH]        = 0.0040f;
    pidconf.DRate[YAW]          = 0.000f;
    ...
    for (axis = 0; axis < 3; axis++) {
        ...
        current_rate    = gyroData[axis];
        rate_d          = current_rate - last_rate[axis];
        last_rate[axis] = current_rate;
        ...
        if (axis == 2) {
            if (rate_i[axis] < -_imaxYaw)
                rate_i[axis] = -_imaxYaw;
            else if (rate_i[axis] > _imaxYaw)
                rate_i[axis] = _imaxYaw;
        } else {
            if (rate_i[axis] < -_imaxPR)
                rate_i[axis] = -_imaxPR;
            else if (rate_i[axis] > _imaxPR)
                rate_i[axis] = _imaxPR;
        }
        i = rate_i[axis];
        ...
        if (deltaT != 0)
            ...
            delta = 0;
        d = pt2FilterApply(&piddFilter[axis], delta * pidconf.DRate[axis]);
...
#define MOTORSTOPVAL 1000
    ...
    for (axis = 1; axis < 4; axis++)
        if (motor[axis] > maxMotor) maxMotor = motor[axis];
    ...
    motorPWMValue[0] = (uint16_t)motor[0] - MOTORSTOPVAL; // 0~1000   右上
    motorPWMValue[1] = (uint16_t)motor[2] - MOTORSTOPVAL; // 左上
...
void pid_init(void)
{
    pt2FilterInit(&piddFilter[0], pt2FilterGain(200, 0.001f));
    pt2FilterInit(&piddFilter[1], pt2FilterGain(200, 0.001f));
    pt2FilterInit(&piddFilter[2], pt2FilterGain(200, 0.001f));
}
