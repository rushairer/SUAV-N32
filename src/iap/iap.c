...
int32_t app_flag_write(uint32_t data, uint32_t start_add)
{
    FLASH_Unlock();
    //
    FLASH_One_Page_Erase(start_add); // 写之前先擦一遍，每次擦2K
...
int32_t app_flash_write(uint32_t *data, uint32_t Flash_address)
{
    ...
    for (i = 0; i < FLASH_PAGE_SIZE / 4; i++) {
        if (FLASH_EOP != FLASH_Word_Program(start_add + i * 4, data[i])) {
            FLASH_Lock();
            //      printf("flash write fail! \r\n");
            //      pages_number = 0;
            return 1;
...
void cfg_save_temp(void)
{
    ...
    FLASH_Unlock();
    FLASH_One_Page_Erase(APP_UPDATE_FLAG_ADDR);

    FLASH_Word_Program(APP_UPDATE_FLAG_ADDR, FLASH_APP_EFFECT_MAGIC_NUMBER);

    for (i = 0; i < sizeof(cfgTemp); i += 4) {
        if (FLASH_EOP != FLASH_Word_Program(ACC_GYRO_DATA_OFFSET_ADDR_TEMP + i, p[i / 4])) {
            FLASH_Lock();
            break;
        }
    }
